...
createTables conn tableNames =
        ...
        where   ptn = getPlayerTableName tableNames  --"player table name"
                ...
                createMatchesTable = ssub "CREATE TABLE IF NOT EXISTS ?(id SERIAL PRIMARY KEY, whichGame INTEGER NOT NULL, dealersHand BIGINT NOT NULL, whichPlayer INTEGER REFERENCES ?, thisPlayersHand BIGINT NOT NULL, playerResult INTEGER NOT NULL)" [mtn, ptn ++ "(whichPlayer)"]
